...
## Nota sobre los cambios solicitados

Estoy en **Explore Mode** — un modo de investigación de código. Puedo analizar y documentar, pero no puedo implementar los cambios. Para ejecutar las modificaciones (eliminar el login y migrar la IA fuera de Lovable), cambiá al **Act Mode** usando el selector de modo en la parte inferior del chat. El archivo `project_info__1.md` (guardado en la raíz del proyecto) contiene el mapa completo de todos los archivos a tocar para ambos cambios, con su orden de prioridad y los riesgos asociados.
